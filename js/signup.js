...
window.onload = async function onLoad() {
    ...
    document.getElementById("regform").onsubmit = async function onSubmit(e) {
        ...
        for (let account of storage) {
            if (account.username === username) throwErr("Tên đăng nhập đã tồn tại.");
            if (account.email === email) throwErr("Email này đã được đăng kí với tài khoản khác.");
        }

        // Tất cả đều hợp lệ. Bắt đầu ghi thông tin tài khoản vào storage.
        storage.push({
            username,
            password,
            email,
            firstname,
            lastname,
            dateofbirth: [ngaysinh, thangsinh, namsinh],
            gender
        });
        localStorage.setItem("accountData", JSON.stringify(storage));
